...
window.Vaadin.Flow['_vaadintheme_camundarest-client_globalCss'] = window.Vaadin.Flow['_vaadintheme_camundarest-client_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_camundarest-client_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_camundarest-client_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_camundarest-client_globalCss'].push(target);
  }
  if (!document['_vaadintheme_camundarest-client_componentCss']) {
    
    document['_vaadintheme_camundarest-client_componentCss'] = true;
  }
